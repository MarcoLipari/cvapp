...
In **Section Library**, use the **CV heading** menu to show all sections or only sections with a selected exported heading, such as Skills or Experience.

Each Section Library item has an internal library name and a separate CV heading. Renaming the internal name keeps every CV link and does not change exported CVs. Editing the CV heading, category, or content updates every CV that still uses that library section and regenerates their Markdown and PDF exports. If updated content no longer fits legibly on one page, the CV remains updated and the app reports which export needs attention.
